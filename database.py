...
user_interests = Table(
    ...
    Column(
        "interest_name", String(100), ForeignKey("interests.name"), primary_key=True
    ),
)
...
def ban_user(db, user_id: int, admin_id: int, reason: Optional[str] = None):
    ...
    if user:
        ...
        admin_action = AdminAction(
            admin_id=admin_id, action_type="ban", target_user_id=user_id, reason=reason
        )
        db.add(admin_action)
        db.flush()
...
def unban_user(db, user_id: int, admin_id: int):
    ...
    if user:
        ...
        # Log admin action
        admin_action = AdminAction(
            admin_id=admin_id, action_type="unban", target_user_id=user_id
        )
        db.add(admin_action)
        db.flush()
...
def get_pending_reports(db):
    ...
    return (
        db.query(UserReport)
        .filter(UserReport.reviewed == False)
    ...
    if user:
        if field == "bio":
            ...
            if len(value) >= 2 and len(value) <= 20:
                existing = (
                    db.query(User)
                    .filter(User.nickname == value, User.user_id != user_id)
                    .first()
                )
                if existing:
                    return False
...
def create_chat_session(db, user_a_id: int, user_b_id: int) -> ChatSession:
    ...
    session = ChatSession(user_a_id=user_a_id, user_b_id=user_b_id)
    db.add(session)
    db.flush()
...
def get_all_user_ids(db) -> List[int]:
    ...
    return [
        user.user_id
        for user in db.query(User.user_id).filter(User.is_banned == False).all()
    ]


def create_broadcast_message(db, admin_id: int, message: str) -> BroadcastMessage:
    """Create a broadcast message record"""
    broadcast = BroadcastMessage(admin_id=admin_id, message=message)
    db.add(broadcast)
    db.flush()

    # Log admin action
    admin_action = AdminAction(
        admin_id=admin_id, action_type="broadcast", broadcast_message=message
    )
    db.add(admin_action)
...
def update_broadcast_stats(db, broadcast_id: int, sent_count: int, failed_count: int):
    ...
    broadcast = (
        db.query(BroadcastMessage).filter(BroadcastMessage.id == broadcast_id).first()
    )
    if broadcast:
